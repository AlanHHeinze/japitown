################################################################################
## Violet Quest 03 — Limpieza del Sábado
################################################################################
## Label narrativo principal y ramificaciones
## Corresponde a: quest_violet_4 (id: violet_questprincipal_4)


## Nota: El mecanismo de fallo (sábado tarde sin completar) está definido en
## config_fallo dentro de quest_violet.rpy y es manejado por el core del sistema.

# =============================================================================
# IMAGENES - Quest 2 Violet
# =============================================================================
...
################################################################################
## LABEL PRINCIPAL — Quest 2 de Violet
################################################################################

label quest_violet_questprincipal_4:
    ...
    #Agregar a Violet limpiando como sprite
    ...
    # Devolver control al jugador — el game_loop mantiene el HUD activo
    # mientras el jugador navega al living
    jump game_loop
...
label violet_quest2_pasillo_interactivo:
    ...
    # Bloquear todo mientras el jugador no interactúe con Violet
    # (el sprite ya está en elementos_escena, el click dispara violet_quest2_escena_pasillo)
    # Solo necesitamos mostrar el HUD con el sprite visible
    
    # El sprite de Violet barriendo se muestra via elementos_escena del sistema de restricción
    # El jugador debe hacer click en él para continuar
    # Este label es llamado con `call expression` desde accion_hotspot_move, return es correcto
    return


################################################################################
## LABEL: Escena del pasillo — Violet barriendo (líneas originales 212-285)
################################################################################
...
label violet_quest2_cocina_interactivo:
    ...
    label .loop_busqueda:
        ...
        if _elemento_cocina == "alacena":
            piensa "Aca esta"
            # Encontró el trapeador, continuar
        else:
            # Click fuera del elemento correcto
            piensa "Alguno de estos muebles debe tener un trapeador"
            jump .loop_busqueda
...
################################################################################
## LABEL: Escena del baño (líneas originales 287-364)
################################################################################

label violet_quest2_escena_banio:
    
    # TODO: Narrativa del baño (líneas 287-364 del script original)
    # Esta sección se completará con el contenido narrativo del baño
    ...
    # Animacion mc levantando a violet
...
label violet_quest2_cierre:
    ...
    # Completar la quest (restaura rutinas, avanza estado)
